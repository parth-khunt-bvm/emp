...
use DB;
class GallerySubmenu extends Model
{
    ...
    public function getdatatable(){
        ...
        $query = GallerySubmenu ::from('gallerysubmenu')
                    ->where("is_deleted","No");
    ...
    public function addSubmenu($data){
        $count = GallerySubmenu::where("name", $data)
       ->count();
       if ($count == 0) {
    ...
    public function getDetail($id){
        return GallerySubmenu::select('id','name')->where("id",$id)->get();
    }
    public function editDetail($request){
        $count = GallerySubmenu::where("name",$request->input('name'))
        ->count();
        if ($count == 0) {
            $obj  = GallerySubmenu::find($request->input('editId'));
    ...
    public function getAllDetails(){
        return GallerySubmenu::select('id','name')
        ->where("is_deleted","No")->get();
    }

    public function  deleteGallerySubmenu($data){
        $obj = GallerySubmenu::find($data['id']);
        $obj->is_deleted = "Yes";
        $obj->updated_at = date("Y-m-d h:i:s");
        return $obj->save();
    }
